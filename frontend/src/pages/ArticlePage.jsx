...
import {
  fetchTrendingArticleById,
  fetchWhatToWatchById,
  fetchTVShowById,
  fetchHomepageArticleById,
  fetchMovieById,
} from "../api/api";
...
const ArticlePage = ({ type }) => {
  ...
  // Map type -> fetch function
  const apiFetchers = {
    trending: () => fetchTrendingArticleById(id),
    watch: () => fetchWhatToWatchById(id),
    tvshow: () => fetchTVShowById(id),
    home: () => fetchHomepageArticleById(id),
  ...
  useEffect(() => {
    const fetchArticle = async () => {
      try {
        const res = await apiFetchers[type]();
        setArticle(res.data);
      } catch (err) {
